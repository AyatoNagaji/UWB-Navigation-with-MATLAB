...
figure(2) 
hold on
plot(d_error,'r'); plot(d_error_min,'b'); 
title('各点の誤差');
plot((len)/2,d_error_ave,'rp'); plot((max_check-min_check)/2,d_error_min_ave,'bp'); 
ylabel('[m]');
hold off

figure(3)
hold on
plot(d_sensor1(1,1:len),'b');
plot(t1(1,1:len).*c);
%plot(t1_env*c,'k');
plot(I1,M1.*c,'o');
plot(I1(1,1):I1(1,length(I1)),t1_min(1,I1(1,1):I1(1,length(I1))).*c,'k');
title('送信局と受信局の距離')
hold off
...
%%大体 d 個前の値でタップ数をkにすれば良い証明
%maxk=30; maxd=50;
%final=zeros(maxk,maxd);
%for k=1:maxk
%for d=1:maxd
%parfor i=1:(len-back-d)
%width=1:i+d;
%[upper,lower]=envelope(t1(1,width).*c,k,'peak');
%sa(d,i)=abs(lower(1,i)-d_sensor1(1,i));
%end
%end
%u=sum(sa.');
%final(k,1:length(u))=u;
%end

%final=-final;
%figure(7)
%[X,Y]=ndgrid(10:30,1:maxd);
%mesh(X,Y,final(10:30,1:maxd));
%hold on
%[C, I] = max(final(:));
%[ii, jj] = ind2sub(size(final), I);
%plot3(ii,jj,C*ones(1,1),'rp');
%xlabel('タップ数'); ylabel('戻り数'); zlabel('-誤差');
%hold off

%figure(8)
%plot(-final(12,(1:50)));
%xlabel('戻り数'); ylabel('誤差[m]');